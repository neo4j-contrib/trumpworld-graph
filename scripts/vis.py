...
def draw_query(session, query, options, physics=False, limit=100):
	...
	data = session.run(query, limit=limit)
	return vis_result(data, options, physics, limit)


def vis_result(data, options, physics=False, limit=100):
	...
	for row in data:
		...
		source_info = get_vis_info(source, source.id)

		if source_info not in nodes:
			nodes.append(source_info)

		if rel is not None:
			target_info = get_vis_info(target, target.id)
...
def draw_graph_query(graph, query, options, physics = False, limit = 100):
