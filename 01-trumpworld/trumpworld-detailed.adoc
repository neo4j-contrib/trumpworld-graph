...
WITH 
{org_org_url} AS url, 
['LOAN','LOBBIED','SALE','SUPPLIER','SHAREHOLDER','LICENSES','AFFILIATED','TIES','NEGOTIATION','INVOLVED','PARTNER','BOUGHT','DEVELOPER','RUSSIAN','TIE','AGREEMENT','MARKETING','SERVICES','DEVELOPER','RECEIVED','MERGER','WORKED_TOGETHER','BACKER','QUESTION','LEASE','BID'] AS terms
LOAD CSV WITH HEADERS FROM url AS row
...
RETURN coalesce(found,type) AS type, count(*) 
ORDER BY count(*) DESC;
----
...
WITH apoc.text.regreplace(toUpper(row.Connection),'\\W+','_') AS type, row, terms
WITH coalesce(head(filter(term IN terms WHERE type CONTAINS term)), case when size(split(type,"_")) <= 2 then type else 'CONNECTED_TO' end) AS type, row
...
We can even try to compute our terms from the data
...
{person_org_url} AS url
LOAD CSV WITH HEADERS FROM url AS row
UNWIND split(apoc.text.regreplace(toUpper(row.Connection),'\\W+','_'),"_") AS part
return part, count(*) 
...
{person_org_url} AS url
LOAD CSV WITH HEADERS FROM url AS row
WITH split(apoc.text.regreplace(toUpper(row.Connection),'\\W+','_'),"_") as parts
UNWIND [idx in range(0,length(parts)-2) | parts[idx..idx+2]] AS part
return part, count(*) 
ORDER BY count(*) DESC LIMIT 30
----
...
WITH 
{person_org_url} AS url,
['BOARD','DIRECTOR','INCOME','PRESIDENT','CHAIR','CEO','PARTNER','OWNER','INVESTOR','FOUNDER','STAFF','DEVELOPER','EXECUTIVE_COMITTEE','EXECUTIVE','FELLOW','BANKER','COUNSEL','ADVISOR','SHAREHOLDER','LIASON','SPEECH','CONNECTED','HIRED','CONSULTED','INVOLVED','APPOINTEE','MANAGER','TRUSTEE','AMBASSADOR','PUBLISHER','LAWYER','SERVICES','CONSULTANT','SECRETARY','CO_RUN','DEGREE','EDITOR','ANALYST','DEAL','LIASON','OFFICER','MARKETING','DONOR','LEGAL','MEMBER','WORKED_FOR','DELEGATE','BACKER','DONATE','COMMANDER','STUDENT','BOUGHT','LED'] AS terms
LOAD CSV WITH HEADERS FROM url AS row
WITH apoc.text.regreplace(toUpper(row.Connection),'\\W+','_') AS type, row, terms
WITH head(filter(term IN terms WHERE type CONTAINS term)) AS found, type, row
RETURN coalesce(found,type) AS type, count(*)
...
WITH apoc.text.regreplace(toUpper(row.Connection),'\\W+','_') AS type, row, terms
WITH coalesce(head(filter(term IN terms WHERE type CONTAINS term)), case when size(split(type,"_")) <= 2 then type else 'INVOLVED_WITH' end) AS type, row
...
WITH apoc.text.regreplace(toUpper(row.Connection),'\\W+','_') AS type, row, terms
WITH coalesce(head(filter(term IN terms WHERE type CONTAINS term)), case when size(split(type,"_")) <= 2 then type else 'RELATED_TO' end) AS type, row
